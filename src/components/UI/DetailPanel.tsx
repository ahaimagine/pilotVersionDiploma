...
const DetailPanel: React.FC<DetailPanelProps> = ({ building, onClose, onBuildRoute }) => {
  ...
  return (
    ...
    >
      <motion.div
        className="absolute left-0 right-0 bottom-0 bg-white dark:bg-gray-900 rounded-t-2xl shadow-lg max-h-[80vh] overflow-y-auto pointer-events-auto"
        initial={{ y: '100%' }}
        animate={{ y: 0 }}
        exit={{ y: '100%' }}
      ...
      >
        <div className="p-4">
          ...
          <div className="flex items-start gap-3 mb-4 text-gray-700 dark:text-gray-300">
          ...
          <div className="flex items-start gap-3 mb-4 text-gray-700 dark:text-gray-300">
            ...
            <div>
              <h3 className="font-semibold text-gray-900 dark:text-white mb-1">{t('workingHours')}</h3>
          ...
          <button
            ...
            className="w-full mt-4 py-3 px-4 bg-primary-600 hover:bg-primary-700 text-white rounded-lg flex items-center justify-center gap-2 transition-colors"
          >
            <Navigation size={20} />
